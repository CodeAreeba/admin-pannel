...
const formatValueForExcel = (value, type = null, field = null) => {
  if (value && typeof value === 'object' && !(value instanceof Date) && !Array.isArray(value)) {
    const extracted = extractInfo(value, field);
    if (extracted !== null) value = extracted;
    else return '';
...
const POSReports = () => {
  ...
  const handleDownloadExcel = (e, report) => {
    e.stopPropagation();
    ...
    try {
        ...
        const excelData = rows.map((row, index) => {
            ...
            headers.forEach(header => {
                let rawValue = row[header.key];
                let formattedValue = formatValueForExcel(rawValue, header.type, header.key);
                rowData[header.label] = formattedValue;
            });
            return rowData;
        });
        ...
        const fileName = `${(report.title || "report").replace(/[^a-z0-9]/gi, '_')}.xlsx`;

        XLSX.writeFile(wb, fileName);

        setSnackbar({ open: true, message: "Excel Downloaded Successfully!", severity: "success" });

    } catch (error) {
        console.error("Excel Generation Error:", error);
        setSnackbar({ open: true, message: "Error generating Excel file", severity: "error" });
    }
  };
          ...
          <Paper elevation={3} sx={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "90%", maxWidth: 1200, maxHeight: "90vh", overflow: "auto", zIndex: 1300, p: 4 }}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
              ...
              <Box display="flex" gap={1}>
                  ...
                  <Button variant="contained" color="success" startIcon={<DownloadIcon />} onClick={(e) => handleDownloadExcel(e, viewingReport)} sx={{ textTransform: "none" }}>
                    Excel
                  </Button>
                  ...
                  <Box display="flex" justifyContent="space-between" alignItems="start">
                    ...
                    <Box display="flex" alignItems="center" gap={1}>
                      ...
                      <Tooltip title="Download PDF">
                        <IconButton onClick={(e) => handleDownloadPDF(e, r)} sx={{ color: "warning.main", border: "1px solid", borderColor: "warning.main", "&:hover": { backgroundColor: "warning.light", color: "white" } }}>
                            <DownloadIcon fontSize="small" />
                        </IconButton>
                      </Tooltip>

                      <Tooltip title="Download Excel">
                        <IconButton onClick={(e) => handleDownloadExcel(e, r)} sx={{ color: "success.main", border: "1px solid", borderColor: "success.main", "&:hover": { backgroundColor: "success.light", color: "white" } }}>
                            <DownloadIcon fontSize="small" />
                        </IconButton>
                      </Tooltip>
